...
class Tabelas:
    ...
    def sufixo(self) -> pd.DataFrame:
        ...
        """        
        DATA_INICIO_SELIC = DateTools.converter_string_para_datetime('01/01/2021')        
        try:
            dt_atualizacao_convertida = DateTools.converter_string_para_datetime(self.data_atualizacao)
        except ValueError as e:
            raise ValueError(f"Erro ao converter 'data_atualizacao': {e}")        
        if dt_atualizacao_convertida < DATA_INICIO_SELIC:
            self.aplicar_selic = False
